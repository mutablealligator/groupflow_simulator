...
class BriteTopo(Topo):
    def __init__(self, brite_filepath, dpid_base = 100, control_net = False):
        # Initialize topology
        Topo.__init__( self )
        ...
        # (Hosts are only added if this is not a control network)
        while in_node_section:
            ...
            dpid = "%0.8X" % (dpid_base + node_id)
            print 'Generating switch and host for ID: ' + str(node_id) + ' DPID: ' + dpid
            switch = self.addSwitch('s' + str(node_id), inband = False, dpid = dpid)
            if not control_net:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) >= 6:
        ...
        topo = BriteTopo(sys.argv[1], 100, False)
        cnet_topo = BriteTopo(sys.argv[1], 500, True)
        hosts = topo.get_host_list()
        start_time = time()
        print 'Simulations started at: ' + str(datetime.now())
        for i in range(0,num_iterations):
            for util_param in util_params:
                p = Process(target=mcastTest, args=(topo, cnet_topo, False, hosts, log_prefix + '_' + ''.join([util_param[0], str(util_param[1])]) + '_' + str(i + first_index) + '.log', util_param[1], util_param[0]))
                sim_start_time = time()
                p.start()
        ...
        cnet_topo = BriteTopo(sys.argv[1])
        hosts = topo.get_host_list()
        mcastTest(topo, cnet_topo, True, hosts)
