...
def mcastTest(topo, interactive = False, hosts = [], log_file_name = 'test_log.log', util_link_weight = 10, link_weight_type = 'linear', controller_placement_metric = LATENCY_METRIC_MIN_AVERAGE_DELAY):
    ...
    # Set the IP of the loopback interface (required when using inNamespace = True)
    # for switch_name in topo.get_switch_list():
    #     net.get(switch_name).cmd('ifconfig lo 192.168.1.' + str(int(switch_name[1:]) + 150))
    ...
    for switch_name in topo.get_switch_list():
        ...
        net.get(switch_name).setHostRoute(switch_ip, 'lo')
        control_path = con_path_map[switch_name]
        while control_path[2]:
            net.get(control_path[1]).setHostRoute(controller_ip, interface_map[control_path[1]][control_path[2][1]])
    ...
    #print 'Installed final hop route for switch ' + controller_switch + ' to controller through interface: ' + interface_map[controller_switch][controller_host]
   # print 'Installed reverse final hop route for host ' + controller_host + ' to controller through interface: ' + interface_map[controller_host][controller_switch]
